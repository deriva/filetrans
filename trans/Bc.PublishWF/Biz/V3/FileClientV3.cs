...
namespace Bc.PublishWF.Biz.V3
{
    public static class FileClientV3
    {
        ...
        public static bool SendFile(string siteno, string fullPath)
        {
            try
            {
                ...
                for (int i = 0; i < PacketCount; i++)
                {
                    ...
                    ControlHelper.AddLabMsg(string.Format("包数{0}=>{1},进度{2}%,耗时:{3},速度:{4}MB/s,预计剩余需:{5}秒", PacketCount, i, (i / (PacketCount * 1.0m) * 100).ToStr("N2"), SecondToStr(stt), speed.ToStr("N2"), SecondToStr(remiantime)));
                ...
                ControlHelper.AddLabMsg(string.Format("包数{0}=>{1},进度{2},耗时:{3},速度:{4}MB/s", PacketCount, PacketCount, "100%", SecondToStr(stt), speed.ToStr("N2")));
                //如果还有多余的数据包，则应该发送完毕！
                if (LastDataPacket != 0)
                {
        ...
        public static bool SendFileV2(string siteno, string fullPath)
        {
            try
            {
                ...
                while (st < allfile.Count)
                {
                    ...
                    st++;
                    ControlHelper.AddToolStripStatusLabel($"{allfile.Count}/{st}");
        ...
        /// <param name="cmd"></param>
        /// <returns></returns>
        public static string CmdPareMsgObj(MsgInfo obj)
        {
            if (obj == null) return ResultHelper.ToFail("无法获取参数").ToResult();

            Init(obj.parm2, obj.parm3);//ip 和端口
            if (clientSocket == null) return ResultHelper.ToFail("失败:Socket无法连接").ToResult();
            var r = TransferFilesV3.SendMsgInfo(clientSocket, obj);
            return ResultHelper.ToResponse(r).ToResult();
